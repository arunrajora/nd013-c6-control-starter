...
int main() {
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer,
    ...
    if (s != "") {
      ...
      // Here, we find the closes point to the current point.
      // I tried the first point, last point and weighted average.
      // But, points near the start are unstable sometimes.
      // Points in the end do not have information for the intermediate states.
      // Hence, the closes point worked the best here.
      int min_index = 0;
      for (int i = 0; i < x_points.size(); i++) {
        double min_distance = std::hypot(x_position - x_points[min_index],
                                         y_position - y_points[min_index]);
        double current_distance =
            std::hypot(x_position - x_points[i], y_position - y_points[i]);
        if (current_distance < min_distance) {
          min_index = i;
        }
      }
      // Here we calculate the yaw of the current position and the closes
      // position in our trajectory.
      double desired_yaw = angle_between_points(
          x_position, y_position, x_points[min_index], y_points[min_index]);
      ...
      // Here, we simply find the difference in current velocity and the
      // velocity required for the closes point.
